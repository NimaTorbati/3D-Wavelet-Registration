...
function [f,ddx,ddy, ddz,imsmall]=nim3D_similarity(main, Xx, Xy, Xz)
...
imsmall=interp3(main.X(:,:,:,1),main.X(:,:,:,2),main.X(:,:,:,3),main.imsmall(:,:,:,1), Xx, Xy, Xz);
im_int2=interp3(main.X(:,:,:,1),main.X(:,:,:,2),main.X(:,:,:,3),main.imsmall(:,:,:,2), Xx, Xy, Xz);
im_int3=interp3(main.X(:,:,:,1),main.X(:,:,:,2),main.X(:,:,:,3),main.imsmall(:,:,:,3), Xx, Xy, Xz);
im_int4=interp3(main.X(:,:,:,1),main.X(:,:,:,2),main.X(:,:,:,3),main.imsmall(:,:,:,4), Xx, Xy, Xz);
clear Xx Xy Xz tmp;
...
ddx=dd.*im_int2; ddx(isnan(ddx))=0;
ddy=dd.*im_int3; ddy(isnan(ddy))=0;
ddz=dd.*im_int4; ddz(isnan(ddz))=0;
